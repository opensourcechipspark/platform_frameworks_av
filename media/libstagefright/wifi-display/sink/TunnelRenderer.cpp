	...
	if(1)
	{
		...
		if(1)
		{
			...
			{	
				...
				if(omx_rs_test == NULL)
					omx_rs_test = fopen("/data/test/omx_rs_test.ts","wb");
				if(omx_rs_test != NULL)
				{
					fwrite(buff,buffer->size(),1,omx_rs_test);
...
void TunnelRenderer::destroyPlayer() {
	...
	if (mSurfaceControl_back == NULL) {
        mSurface_back.clear();
        mSurfaceControl_back.clear();

        mComposerClient_back->dispose();
        mComposerClient_back.clear();
    }
}
