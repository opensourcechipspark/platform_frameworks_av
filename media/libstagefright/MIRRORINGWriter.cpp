...
status_t MIRRORINGWriter::addSource(const sp<MediaSource> &source) {
    ...
    if (strcasecmp(mime, MEDIA_MIMETYPE_VIDEO_AVC)
		...
		mRkOn2Encoder = static_cast <RkOn2Encoder *>(ptr);
		mRkOn2Encoder->Set_Flag((rx_flag==1)?2:1,(rx_flag==1)?1:0);
    }
...
void* MIRRORINGWriter::decideByUnsendData() {
	...
	while (end_flag == 0 && mRkOn2Encoder != NULL) {
//		usleep(1000000ll);
		usleep(985000ll);
...
*/
		...
		//set MaxRestSize
		mRkOn2Encoder->get_encoder_param(&mAVCEncParams);
		...
		if (AAData.desktopFlag) {
			...
			mRkOn2Encoder->set_encoder_param(&mAVCEncParams);
...
status_t MIRRORINGWriter::setAvcencBitRate(int64_t bitrate) {
	...
	err = mRkOn2Encoder->get_encoder_param(&mAVCEncParams);
	if (err != OK) {
	...
	err = mRkOn2Encoder->set_encoder_param(&mAVCEncParams);
	if(err != OK){
