...
#if WRITE_FILE
    LOGE("create file: /sdcard/aac.dat");	
    aacFp = fopen("/sdcard/aac.dat","wb");	
    if(aacFp)		
        LOGE("------>>create file success");
    else
...
void SoftAAC::onQueueFilled(OMX_U32 portIndex) {
    ...
    if (portIndex == 0 && mInputBufferCount == 0) {
        ...
        if (err != MP4AUDEC_SUCCESS) {
			...
			if(mUseFaadDecoder == 1)
	        {
				...
				if (err != 0)
				{
					LOGE("NeAACDecInit2 failed: err = %d.\n", err);
					NeAACDecClose(hAac);
					hAac = NULL;
				...
				LOGI("samplerate = %d, channels = %d", samplerate, channels);
			}else {
	            mSignalledError = true;
	            notify(OMX_EventError, OMX_ErrorUndefined, err, NULL);
	            return;
    ...
    while (!inQueue.empty() && !outQueue.empty()) {
		...
		if(mUseFaadDecoder == 0) {
			...
			/*if(hInfo.bytesconsumed < inHeader.nFilledLen) {
				LOGW("Warning: input buffer not fully consumed. Is that OK?\n");
			}*/
			if(hInfo.channels > 2) {
				LOGW("Now do not support channels > 2.\n");
				hInfo.error = -1;
			}
			if ((hInfo.error == 0) && (hInfo.samples > 0))
			{
				...
				LOGE("NeAACDecDecode error: error = %d.\n", hInfo.error);
				decoderErr = MP4AUDEC_INVALID_FRAME;
			} else {
				// decode ok, but no samples output
				decoderErr = MP4AUDEC_SUCCESS;
			}
